...
@Component({
  ...
  templateUrl: './home-clients.page.html',
  styleUrls: ['./home-clients.page.scss'],
  standalone: false
})
export class HomeClientsPage implements OnInit {


  years: number[] = [2025, 2024, 2023, 2022, 2021, 2020, 2019, 2018, 2017, 2016, 2015, 2014, 2013, 2012, 2011, 2010];
  selectedYear: number = new Date().getFullYear();

  @ViewChild('doughnutChartAdmin') doughnutChart?: BaseChartDirective;
  @ViewChild('lineChartAdmin') lineChart?: BaseChartDirective;
  ...
  pieChartData: ChartData<'line', number[], string | string[]> = {
    labels: [],
    datasets: [
      {
        label: 'Finalizados',
  ...
  citaChartType: ChartType = 'doughnut';
  citaChartData: ChartData<'doughnut'> = {
    labels: [],
    datasets: [
      {
        data: [],
        backgroundColor: ['#42A5F5', '#FFA726', '#66BB6A', '#FF7043', '#AB47BC', '#FFCA28', '#8D6E63', '#78909C', '#B39DDB', '#FF8A65'],
      }
    ]
  };

  citaChartOptions: any = {
    responsive: true,
    plugins: {
  ...
  async chargeInformation() {
    ...
    this.dashboardService.getDashboardByAdministrator({
      entityId: clientId,
